...
public class MappingReader {
	...
	public XmlDocument readMappingDocument(EntityResolver entityResolver, InputSource source, Origin origin) {
		...
		try {
		...
		catch ( Exception orm21Problem ) {
			if ( LOG.isDebugEnabled() ) {
				LOG.debugf( "Problem parsing XML using orm 2.1 xsd : %s", orm21Problem.getMessage() );
			}
			failure = orm21Problem;
			errorHandler.reset();
			...
			if ( document != null ) {
				...
				try {
				...
				catch ( Exception orm20Problem ) {
					if ( LOG.isDebugEnabled() ) {
						LOG.debugf( "Problem parsing XML using orm 2.0 xsd : %s", orm20Problem.getMessage() );
					}
					failure = orm20Problem;
					errorHandler.reset();
					...
					if ( document != null ) {
						...
						try {
							...
							if ( LOG.isDebugEnabled() ) {
								LOG.debugf( "Problem parsing XML using orm 1.0 xsd : %s", orm1Problem.getMessage() );
							}
						}
					}
				}
